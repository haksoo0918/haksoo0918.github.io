...
- **프로젝트 썸네일 이미지 에셋 등록**:
  - 각 프로젝트 카드별 16:9 썸네일 이미지(계산기, 시작페이지, 비트코인 퀀트, 블로그) 등록 완료 (`assets/images/projects/`).
- **프로젝트 운영 가이드라인 수립**:
  - 개발 프로토콜(`GEMINI.md`) 및 제품 요구사항 정의서(`PRD.md`) 작성.

### 변경됨 (Changed)
- **오픈그래프(OG) 대표 공유 이미지 교체**:
  - 소셜 링크 공유 시 표시되는 대표 이미지(`assets/images/og-image.png`)를 새로 교체 반영.
- **프로젝트 실서비스 링크 및 정적 마크업 동기화**:
  - 계산기, 시작페이지, 비트코인 퀀트, 블로그의 실제 배포 URL 및 깃헙 저장소 주소를 `data/projects.json`, `index.html` 정적 사전 렌더링 카드, `main.js` 오프라인 폴백 데이터에 일괄 동기화.
